import type { LinesLabel } from "./types";

export const LINES_LABEL_PREFIX = "lines/";

export const LINES_LABELS: readonly LinesLabel[] = [
    {
        name: "lines/<10",
        maxValue: 9,
        color: "b7eb8f",
        description: "Pull request changes fewer than 10 lines",
    },
    {
        name: "lines/10-49",
        maxValue: 49,
        color: "73d13d",
        description: "Pull request changes 10 to 49 lines",
    },
    {
        name: "lines/50-199",
        maxValue: 199,
        color: "d3d74b",
        description: "Pull request changes 50 to 199 lines",
    },
    {
        name: "lines/200-499",
        maxValue: 499,
        color: "fbca04",
        description: "Pull request changes 200 to 499 lines",
    },
    {
        name: "lines/500-999",
        maxValue: 999,
        color: "f9a825",
        description: "Pull request changes 500 to 999 lines",
    },
    {
        name: "lines/1000+",
        maxValue: Infinity,
        color: "d93f0b",
        description: "Pull request changes at least 1,000 lines",
    },
] as const;

export function getLinesLabel(changedLines: number): string {
    if (!Number.isFinite(changedLines) || changedLines < 0) {
        throw new RangeError(
            "changedLines must be a non-negative finite number",
        );
    }

    const label = LINES_LABELS.find((l) => changedLines <= l.maxValue);

    if (label != null) {
        return label.name;
    }

    return LINES_LABELS[LINES_LABELS.length - 1].name;
}
